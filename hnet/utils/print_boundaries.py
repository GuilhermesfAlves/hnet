def print_boundary_tokens(input_ids, bpred_output, tokenizer):
    ...
    for stage, routing in enumerate(bpred_output):
        ...
        for batch_idx in range(input_ids.shape[0]):
            ...
            for group, boundary in zip(display_groups, display_boundaries):
                ...
                new_stage_groups.append(current_group.copy())
                current_group = []
            ...
            if current_group:
                chunk_ids = [original_ids[batch_idx][idx] for idx in current_group]
                chunk_tokens = tokenizer.convert_ids_to_tokens(chunk_ids)
                chunk_text = tokenizer.decode(chunk_ids, skip_special_tokens=False)
